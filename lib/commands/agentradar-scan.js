...
/**
 * Calculate exit code based on risk level and AgentRadar findings
 */
function calculateExitCode(riskLevel, result, failOnHigh, failOnMedium, failOnLow) {
  ...
  // Also fail if AgentRadar found critical issues
  if (result.agentradar?.enabled) {
    const summary = result.agentradar.summary || {};
    if (summary.totalVulnerabilities > 0 || summary.totalPaths > 0) {
      // Check if any vulnerabilities are critical
      const vulnerabilities = result.agentradar.vulnerabilities || [];
...
/**
 * Handle AgentRadar scan command
 * Scans agents/registries and submits to API
 */
export async function handleAgentRadarScanCommand({
  input,
  token,
  verbose,
  json,
  ...
  if (!json) {
    consola.info(`Running AgentRadar scan with input: ${input}`);
  ...
  // Submit to AgentRadar API endpoint
  if (!json) {
    consola.info("Submitting to AgentRadar API...");
  }

  try {
    const response = await apiClient.post("/api/agentradar/scan", agentData);
    ...
    if (result.error || !result.success) {
      if (json) {
        ...
        consola.error("AgentRadar scan failed:", result.error || result.message);
      }
      process.exit(1);
    }
    ...
    if (json) {
      ...
      consola.success("AgentRadar scan completed successfully");
      consola.info(`Scan ID: ${result.scan_id}`);

      if (result.agentradar?.enabled) {
        const summary = result.agentradar.summary || {};
        consola.info("AgentRadar Analysis Summary:");
      ...
      if (result.scan_id) {
        consola.info("\nView detailed results: https://smart.mcpshark.sh/dashboard/agentradar");
      }
    }
    ...
    if (error.response) {
      ...
      if (json) {
        console.error(
          JSON.stringify({ error: "AgentRadar scan failed", message: errorMessage }, null, 2)
        );
      } else {
        consola.error("AgentRadar scan failed:", errorMessage);
      }
    }
    process.exit(1);
  }
