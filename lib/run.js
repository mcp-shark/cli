...
import { MCPError, getErrors, isError } from "./error.js";
...
export async function runServer({ logger, name, config }) {
  ...
  if (isError(transport)) {
    return new RunError(`Error creating transport for server ${name}`, transport.error);
  }

  // Create client
  const client = await createClient({ transport });
  if (isError(client)) {
    return new RunError(`Error creating client for server ${name}`, client.error);
  }
  ...
  if (isError(closeResult)) {
    return new RunError(`Error closing client for server ${name}`, closeResult.error);
  }

  // Check for errors
  const errors = getErrors(results);
  if (errors.length > 0) {
    return new RunError(`Errors occurred while running requests for server ${name}`, null, errors);
  }
...
export async function runAllServers(logger, parsedConfig) {
  ...
  const results = await Promise.all(
    Object.entries(configs).map(([name, config]) => runServer({ logger, name, config }))
  );

  const errors = getErrors(results.flat());
  if (errors.length > 0) {
    return new RunError("Errors occurred while running all servers", null, errors);
  }
  return results.flat();
}
