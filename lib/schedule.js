 ...
 */
export async function scheduleScans(apiClient, serverResults, logger = null, json = false) {
  // Ensure we have an array
  const servers = Array.isArray(serverResults) ? serverResults : [serverResults];

  // Show progress for each server
  if (!json && logger) {
    logger.info(`Submitting ${servers.length} scan(s) to security analysis API...`);
  }
  ...
  const scanPromises = servers.map(async (serverResult, index) => {
    ...
    if (!json && logger) {
      if (isError(scanResult)) {
        logger.error(`[${index + 1}/${servers.length}] Scan failed for server: ${serverName}`);
      } else {
        logger.success(`[${index + 1}/${servers.length}] Scan completed for server: ${serverName}`);
      }
    }
