...
- **NEVER use dynamic imports** (`await import()`, `import()`, `require()`) - ALWAYS use static imports at the top of the file
- **ALL imports MUST be at the top of the file** - Never inside functions, conditionals, try-catch blocks, or any other code blocks
- **ALL imports MUST be declared before any other code** - No code (except comments) should appear before import statements
- Dynamic imports are strictly forbidden - if you need conditional imports, restructure the code to use static imports
- Example:
...
### 10.1. Nested Function Declarations

- **NEVER declare functions inside other functions**
- **ALWAYS extract nested functions to module level**
- This applies to both `function` declarations and arrow function assignments
- Functions should be declared at the top level of the module
- Example:

  ```javascript
  // ❌ WRONG - Function declared inside another function
  export async function POST(request) {
    const validateData = async (body) => {
      // validation logic
    };
    const result = await validateData(request.body);
  }

  // ❌ WRONG - Function declaration inside function
  export async function POST(request) {
...
- **Exception:** Arrow functions in `useEffect` callbacks are allowed (React pattern):
  ```javascript
  // ✅ CORRECT - Arrow function in useEffect is allowed
  useEffect(() => {
    const loadData = async () => {
      const data = await fetchData();
      setData(data);
