...
# Test AgentRadar scan with SQLite MCP Server
...
echo "Testing AgentRadar scan with SQLite MCP Server..."
...
if command -v jq &> /dev/null; then
  ./cli agentradar scan -i "$TEMP_DIR/sqlite-agent.json" --token="$TOKEN" --json | jq '.'
else
  ./cli agentradar scan -i "$TEMP_DIR/sqlite-agent.json" --token="$TOKEN" --json
fi
